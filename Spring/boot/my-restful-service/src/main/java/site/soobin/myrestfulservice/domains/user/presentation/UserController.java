package site.soobin.myrestfulservice.domains.user.presentation;
...
public class UserController {
  ...
  @GetMapping("")
  public List<XUser> retreiveAllUsers() {
  ...
  @GetMapping("/{id}")
  public EntityModel<XUser> retreiveUser(@PathVariable int id) {
    XUser findUser = userDaoService.findOne(id);
    if (findUser == null) {
      throw new ApiBusinessException(UserErrorSpec.USER_NOT_FOUND);
    }
    EntityModel<XUser> entityModel = EntityModel.of(findUser);
  ...
  @PostMapping("")
  public ResponseEntity<XUser> createUser(@Valid @RequestBody XUser user) {
     ...
     */
    XUser savedUser = userDaoService.save(user);
    URI location =
  ...
  public ResponseEntity<Void> deleteUser(@PathVariable int id) {
    XUser deleteUser = userDaoService.deleteById(id);
    if (deleteUser == null) {
      throw new ApiBusinessException(UserErrorSpec.USER_NOT_FOUND);
    }
